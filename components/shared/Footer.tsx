"use client";
import { FaLocationArrow } from "react-icons/fa6";

import { socialMedia } from "@/data/home/socials";
import MagicButton from "../ui/MagicButton";

const Footer = () => {
  const goToSocialMedia = (link: string) => {
    window.open(link, "_blank");
  };

  return (
    <footer
      className="w-full pt-20 pb-10 bg-footer bg-cover bg-foreground dark:bg-transparent"
      id="contact"
    >
      <div className="flex flex-col items-center">
        <h1 className="heading lg:max-w-[45vw] text-white">
          Ready to take <span className="text-purple-500">your</span> digital
          presence to the next level?
        </h1>
        <p className="text-white md:mt-10 my-5 text-center">
          Reach out to me today and let&apos;s discuss how I can help you
          achieve your goals.
        </p>
        <a href="mailto:abrarme118@gmail.com">
          <MagicButton
            title="Let's get in touch"
            icon={<FaLocationArrow />}
            position="right"
            otherClasses="bg-background text-foreground"
          />
        </a>
      </div>
      <div className="flex mt-5 md:mb-0 mb-10 md:mt-16 md:flex-row flex-col justify-between items-center md:px-10">
        <p className="md:text-base text-sm md:font-normal font-light text-white">
          Copyright © 2024 Abrar Mahir Esam
        </p>

        <div className="flex items-center md:gap-3 gap-6">
          {socialMedia.map((info) => (
            <div
              key={info.id}
              className="w-10 h-10 cursor-pointer flex justify-center items-center backdrop-filter 
              backdrop-blur-lg saturate-180 bg-opacity-75 bg-black-200 rounded-lg border border-black-300"
              onClick={() => {
                goToSocialMedia(info.link);
              }}
              onKeyUp={() => {
                goToSocialMedia(info.link);
              }}
            >
              <img src={info.img} alt="icons" width={20} height={20} />
            </div>
          ))}
        </div>
      </div>
    </footer>
  );
};

export default Footer;
